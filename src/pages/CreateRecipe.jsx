...
function Navigation({ recipeCreationData }) {
  ...
  const [loadingType, setLoadingType] = useState('generation');
  ...
  const handleIngredientSubmit = async () => {
    try {
      ...
      parsedRecipes = parsedRecipes.map((recipe, idx) => ({
        ...recipe,
        openaiPromptId: `${openaiPromptId}-${idx}`,
      }));
  ...
  const handleRecipeSubmit = async (recipes) => {
    try {
      setIsLoading(true);
      setIsComplete(false);
